...
- Partials needing 100%: no-undef (91/97), no-use-before-define (276/354)
...
| `no-use-before-define` | ✓ | Shipped 2026-05-15 (276/354 fixtures, 78.0%; flags self-init / TDZ patterns including parameter defaults, destructuring, for-in/of iterable self-ref, class heritage self-ref; nested class heritage clauses correctly checked against the owning class; references inside type positions (`typeof X` in `interface`, `type`) are skipped under `IgnoreTypeReferences: true`) |
