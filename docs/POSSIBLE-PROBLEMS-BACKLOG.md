...
- Partials needing 100%: no-undef (92/97), no-use-before-define (350/354)
...
| `no-use-before-define` | ✓ | Shipped 2026-05-15 (350/354 fixtures, 98.9%; flags self-init / TDZ patterns; eager class contexts (heritage clause, computed name, static field, static block) override hoisting options when not nested in a deferred body; `new X()` at non-deferred positions forces flagging regardless of Classes option; `with (...)` blocks fall back to source-file binding lookup; declarationKind covers Enums and Modules; Variables option's deferred-body suppression is scoped to deferred bodies that do not enclose the declaration; AllowNamedExports and IgnoreTypeReferences honored) |
