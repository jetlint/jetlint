...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nosecrets"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-secrets.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nse-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nosecrets.New()},
		map[string]wrapperlint.Severity{"no-secrets": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-secrets" {
			count++
		}
	}
	return count, nil
