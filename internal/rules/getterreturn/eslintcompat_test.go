...
func TestGetterReturn_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/getter-return.json")
...
func optionsForCase(rawOptions []any) getterreturn.Options {
	if len(rawOptions) == 0 {
		return getterreturn.Options{}
	}
	body, err := json.Marshal(rawOptions[0])
	if err != nil {
		return getterreturn.Options{}
	}
	opts, err := getterreturn.OptionsFromJSON(body)
	if err != nil {
		return getterreturn.Options{}
	}
	return opts
...
func runEslintCase(t *testing.T, code string, opts getterreturn.Options) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-gr-compat")
	if err != nil {
		return 0, err
	}
