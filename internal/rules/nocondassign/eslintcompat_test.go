...
func TestNoCondAssign_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-cond-assign.json")
	...
	for i, c := range fx.Cases {
		...
		count, runErr := runEslintCase(t, c.Code, nocondassign.OptionsFromJSON(raw))
		if runErr != nil {
			failed++
		...
		t.Logf("MISMATCH [%s #%d options=%v] count=%d\n--- code ---\n%s\n--- end ---",
			valid, i, c.Options, count, c.Code)
	}
	total := passed + failed
	pct := 0.0
	if total > 0 {
...
func runEslintCase(t *testing.T, code string, opts nocondassign.Options) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nca-compat")
	if err != nil {
		return 0, err
	}
