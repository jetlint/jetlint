package novueduplicatekeys_test
...
func TestNoVueDuplicateKeys_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-vue-duplicate-keys.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nvdk-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{novueduplicatekeys.New()},
		map[string]wrapperlint.Severity{"no-vue-duplicate-keys": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-vue-duplicate-keys" {
			count++
		}
	}
	return count, nil
