package usehookattoplevel_test
...
func TestUseHookAtTopLevel_EslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-hook-at-top-level.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-uhtl-")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{usehookattoplevel.New()},
		map[string]wrapperlint.Severity{"use-hook-at-top-level": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-hook-at-top-level" {
			count++
		}
	}
	return count, nil
