package nounresolvedimports_test
...
func TestNoUnresolvedImports_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-unresolved-imports.json")
...
func runCase(t *testing.T, c eslintcompat.Case) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nui-")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounresolvedimports.New()},
		map[string]wrapperlint.Severity{"no-unresolved-imports": wrapperlint.SeverityError},
	)
	count := 0
	wantPath := filepath.Join(dir, mainPath)
	for _, d := range eng.Lint(prog) {
		if d.RuleID != "no-unresolved-imports" {
			continue
		}
