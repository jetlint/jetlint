...
func TestRequireAwait_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/require-await.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "require-await")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{requireawait.New()},
		map[string]wrapperlint.Severity{"require-await": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "require-await" {
			count++
		}
	}
	return count, nil
