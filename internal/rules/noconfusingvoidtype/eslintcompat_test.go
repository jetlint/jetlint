package noconfusingvoidtype_test
...
const ruleID = "no-confusing-void-type"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New([]engine.Rule{noconfusingvoidtype.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
