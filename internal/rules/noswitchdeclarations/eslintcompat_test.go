package noswitchdeclarations_test
...
func TestNoSwitchDeclarations_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-switch-declarations.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nsd-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noswitchdeclarations.New()},
		map[string]wrapperlint.Severity{"no-switch-declarations": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-switch-declarations" {
			count++
		}
	}
	return count, nil
