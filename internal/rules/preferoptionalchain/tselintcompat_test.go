package preferoptionalchain_test
...
func TestPreferOptionalChain_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/prefer-optional-chain.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "prefer-optional-chain")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{preferoptionalchain.New()},
		map[string]wrapperlint.Severity{"prefer-optional-chain": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "prefer-optional-chain" {
			count++
		}
	}
	return count, nil
