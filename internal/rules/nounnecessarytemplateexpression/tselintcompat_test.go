package nounnecessarytemplateexpression_test
...
func TestNoUnnecessaryTemplateExpression_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unnecessary-template-expression.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unnecessary-template-expression")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounnecessarytemplateexpression.New()},
		map[string]wrapperlint.Severity{"no-unnecessary-template-expression": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unnecessary-template-expression" {
			count++
		}
	}
	return count, nil
