package nononoctaldecimalescape_test
...
func TestNoNonoctalDecimalEscape_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-nonoctal-decimal-escape.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nnde-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nononoctaldecimalescape.New()},
		map[string]wrapperlint.Severity{"no-nonoctal-decimal-escape": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-nonoctal-decimal-escape" {
			count++
		}
	}
	return count, nil
