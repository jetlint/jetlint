package nonestedcomponentdefinitions_test
...
func TestNoNestedComponentDefinitions_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-nested-component-definitions.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nncd-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nonestedcomponentdefinitions.New()},
		map[string]wrapperlint.Severity{"no-nested-component-definitions": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-nested-component-definitions" {
			count++
		}
	}
	return count, nil
