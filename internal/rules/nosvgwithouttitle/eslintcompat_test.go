package nosvgwithouttitle_test
...
const ruleID = "no-svg-without-title"
...
func TestEslintCompatibility(t *testing.T) {
	...
	if failed > 0 {
		t.Fatalf("expected 100%% pass rate, got %d/%d", passed, passed+failed)
	}
}

func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-svg")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New([]engine.Rule{nosvgwithouttitle.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
