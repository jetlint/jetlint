package usesolidforcomponent_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/usesolidforcomponent"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-solid-for-component.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-usf-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{usesolidforcomponent.New()},
		map[string]wrapperlint.Severity{"use-solid-for-component": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-solid-for-component" {
			count++
		}
	}
	return count, nil
