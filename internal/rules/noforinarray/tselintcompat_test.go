...
func TestNoForInArray_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-for-in-array.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-for-in-array")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noforinarray.New()},
		map[string]wrapperlint.Severity{"no-for-in-array": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-for-in-array" {
			count++
		}
	}
	return count, nil
