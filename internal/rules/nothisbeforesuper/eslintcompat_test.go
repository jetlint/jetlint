...
func TestNoThisBeforeSuper_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-this-before-super.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ntbs-compat")
	if err != nil {
		return 0, err
	}
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-this-before-super" {
			count++
		}
	}
	return count, nil
