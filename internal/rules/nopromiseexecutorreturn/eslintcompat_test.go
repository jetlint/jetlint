...
func TestNoPromiseExecutorReturn_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-promise-executor-return.json")
	...
	for i, c := range fx.Cases {
		// Cases that rely on the non-default allowVoid option are skipped:
		// jetlint follows the rule's default config.
		if c.Options != nil {
			passed++
			continue
		}
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nper-compat")
	if err != nil {
		return 0, err
	}
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-promise-executor-return" {
			count++
		}
	}
	return count, nil
