package useunknownincatchcallbackvariable_test
...
func TestUseUnknownInCatchCallbackVariable_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/use-unknown-in-catch-callback-variable.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "use-unknown-in-catch-callback-variable")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{useunknownincatchcallbackvariable.New()},
		map[string]wrapperlint.Severity{"use-unknown-in-catch-callback-variable": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-unknown-in-catch-callback-variable" {
			count++
		}
	}
	return count, nil
