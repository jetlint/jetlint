package preferstringstartsendswith_test
...
func TestPreferStringStartsEndsWith_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/prefer-string-starts-ends-with.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "prefer-string-starts-ends-with")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	var passed, failed int
	for _, c := range cases {
		opts := preferstringstartsendswith.DefaultOptions()
		if v, ok := c.Options["allowSingleElementEquality"].(string); ok {
...
func runCase(t *testing.T, code string, opts preferstringstartsendswith.Options) (int, error) {
	t.Helper()
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{preferstringstartsendswith.NewWithOptions(opts)},
		map[string]wrapperlint.Severity{"prefer-string-starts-ends-with": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "prefer-string-starts-ends-with" {
			count++
		}
	}
	return count, nil
