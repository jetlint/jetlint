package noglobaldirnamefilename_test
...
func TestNoGlobalDirnameFilename_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-global-dirname-filename.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ngdf-compat")
	if err != nil {
		return 0, err
	}
	...
	// Use .tsx so JSX-bearing cases parse correctly.
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noglobaldirnamefilename.New()},
		map[string]wrapperlint.Severity{"no-global-dirname-filename": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-global-dirname-filename" {
			count++
		}
	}
	return count, nil
