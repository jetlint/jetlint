package nounsafeunaryminus_test
...
func TestNoUnsafeUnaryMinus_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unsafe-unary-minus.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unsafe-unary-minus")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	...
	for _, c := range cases {
		...
		t.Logf("FAIL [%s #%d] expected=%d actual=%d\n--- code ---\n%s\n--- end ---",
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounsafeunaryminus.New()},
		map[string]wrapperlint.Severity{"no-unsafe-unary-minus": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unsafe-unary-minus" {
			count++
		}
	}
	return count, nil
