package noaccumulatingspread_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/noaccumulatingspread"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-accumulating-spread.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nas-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noaccumulatingspread.New()},
		map[string]wrapperlint.Severity{"no-accumulating-spread": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-accumulating-spread" {
			count++
		}
	}
	return count, nil
