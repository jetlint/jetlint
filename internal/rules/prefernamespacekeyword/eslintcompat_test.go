package prefernamespacekeyword_test
...
const ruleID = "prefer-namespace-keyword"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New([]engine.Rule{prefernamespacekeyword.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
