package nounsafetypeassertion_test
...
func TestNoUnsafeTypeAssertion_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unsafe-type-assertion.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unsafe-type-assertion")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounsafetypeassertion.New()},
		map[string]wrapperlint.Severity{"no-unsafe-type-assertion": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unsafe-type-assertion" {
			count++
		}
	}
	return count, nil
