package usegooglefontpreconnect_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/usegooglefontpreconnect"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-google-font-preconnect.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ugp-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{usegooglefontpreconnect.New()},
		map[string]wrapperlint.Severity{"use-google-font-preconnect": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-google-font-preconnect" {
			count++
		}
	}
	return count, nil
