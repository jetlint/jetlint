...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nobarrelfile"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-barrel-file.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nbf-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nobarrelfile.New()},
		map[string]wrapperlint.Severity{"no-barrel-file": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-barrel-file" {
			count++
		}
	}
	return count, nil
