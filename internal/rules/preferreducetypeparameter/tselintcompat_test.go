package preferreducetypeparameter_test
...
func TestPreferReduceTypeParameter_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/prefer-reduce-type-parameter.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "prefer-reduce-type-parameter")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{preferreducetypeparameter.New()},
		map[string]wrapperlint.Severity{"prefer-reduce-type-parameter": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "prefer-reduce-type-parameter" {
			count++
		}
	}
	return count, nil
