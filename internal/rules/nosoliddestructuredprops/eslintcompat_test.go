package nosoliddestructuredprops_test
...
func TestNoSolidDestructuredProps_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-solid-destructured-props.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nsdp-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nosoliddestructuredprops.New()},
		map[string]wrapperlint.Severity{"no-solid-destructured-props": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-solid-destructured-props" {
			count++
		}
	}
	return count, nil
