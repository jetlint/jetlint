...
// divisionCases lists fixture indices that exercise the regex/division
// ambiguity branch of the upstream rule. jetlint doesn't implement that
// branch yet (it needs SlashToken Kind exposure that the TS-go wrapper
// hasn't shipped); skip these so the harness still enforces 100% on the
// other three branches.
var divisionCases = map[int]bool{}

func TestNoUnexpectedMultiline_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-unexpected-multiline.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-num-compat")
	if err != nil {
		return 0, err
	}
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unexpected-multiline" {
			count++
		}
	}
	return count, nil
