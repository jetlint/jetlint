package noinvalidbuiltininstantiation_test
...
func TestNoInvalidBuiltinInstantiation_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-invalid-builtin-instantiation.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nibi-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noinvalidbuiltininstantiation.New()},
		map[string]wrapperlint.Severity{"no-invalid-builtin-instantiation": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-invalid-builtin-instantiation" {
			count++
		}
	}
	return count, nil
