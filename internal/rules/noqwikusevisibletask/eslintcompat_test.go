package noqwikusevisibletask_test
...
func TestNoQwikUseVisibleTask_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-qwik-use-visible-task.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nquvt-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noqwikusevisibletask.New()},
		map[string]wrapperlint.Severity{"no-qwik-use-visible-task": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-qwik-use-visible-task" {
			count++
		}
	}
	return count, nil
