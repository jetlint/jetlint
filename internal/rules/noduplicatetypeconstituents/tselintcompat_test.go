package noduplicatetypeconstituents_test
...
func TestNoDuplicateTypeConstituents_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-duplicate-type-constituents.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-duplicate-type-constituents")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noduplicatetypeconstituents.New()},
		map[string]wrapperlint.Severity{"no-duplicate-type-constituents": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-duplicate-type-constituents" {
			count++
		}
	}
	return count, nil
