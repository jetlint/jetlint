package notypeonlyimportattributes_test
...
func TestNoTypeOnlyImportAttributes_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-type-only-import-attributes.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ntoia-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{notypeonlyimportattributes.New()},
		map[string]wrapperlint.Severity{"no-type-only-import-attributes": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-type-only-import-attributes" {
			count++
		}
	}
	return count, nil
