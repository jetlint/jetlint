package noredundantusestrict_test
...
const ruleID = "no-redundant-use-strict"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New([]engine.Rule{noredundantusestrict.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
