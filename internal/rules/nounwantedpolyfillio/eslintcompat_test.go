package nounwantedpolyfillio_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nounwantedpolyfillio"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-unwanted-polyfillio.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nup-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounwantedpolyfillio.New()},
		map[string]wrapperlint.Severity{"no-unwanted-polyfillio": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unwanted-polyfillio" {
			count++
		}
	}
	return count, nil
