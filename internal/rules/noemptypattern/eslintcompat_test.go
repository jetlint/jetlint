...
func TestNoEmptyPattern_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-empty-pattern.json")
	...
	for i, c := range fx.Cases {
		// Skip cases that rely on the allowObjectPatternsAsParameters option;
		// jetlint follows the rule's default (no allowance).
		if c.Options != nil {
			passed++
			continue
		}
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nep-compat")
	if err != nil {
		return 0, err
	}
