package nounnecessarytypeparameters_test
...
func TestNoUnnecessaryTypeParameters_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unnecessary-type-parameters.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unnecessary-type-parameters")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounnecessarytypeparameters.New()},
		map[string]wrapperlint.Severity{"no-unnecessary-type-parameters": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unnecessary-type-parameters" {
			count++
		}
	}
	return count, nil
