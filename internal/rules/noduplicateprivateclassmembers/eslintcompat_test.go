package noduplicateprivateclassmembers_test
...
func TestNoDuplicatePrivateClassMembers_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-duplicate-private-class-members.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ndpcm-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noduplicateprivateclassmembers.New()},
		map[string]wrapperlint.Severity{"no-duplicate-private-class-members": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-duplicate-private-class-members" {
			count++
		}
	}
	return count, nil
