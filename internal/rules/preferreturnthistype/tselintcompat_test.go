package preferreturnthistype_test
...
func TestPreferReturnThisType_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/prefer-return-this-type.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "prefer-return-this-type")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{preferreturnthistype.New()},
		map[string]wrapperlint.Severity{"prefer-return-this-type": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "prefer-return-this-type" {
			count++
		}
	}
	return count, nil
