package usejsxkeyiniterable_test
...
func TestUseJsxKeyInIterable_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-jsx-key-in-iterable.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ujki-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{usejsxkeyiniterable.New()},
		map[string]wrapperlint.Severity{"use-jsx-key-in-iterable": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-jsx-key-in-iterable" {
			count++
		}
	}
	return count, nil
