...
func TestPreferReadonly_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/prefer-readonly.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "prefer-readonly")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
