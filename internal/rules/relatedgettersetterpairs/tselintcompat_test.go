package relatedgettersetterpairs_test
...
func TestRelatedGetterSetterPairs_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/related-getter-setter-pairs.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "related-getter-setter-pairs")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{relatedgettersetterpairs.New()},
		map[string]wrapperlint.Severity{"related-getter-setter-pairs": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "related-getter-setter-pairs" {
			count++
		}
	}
	return count, nil
