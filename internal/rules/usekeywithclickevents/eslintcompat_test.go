package usekeywithclickevents_test
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-key-with-click-events.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-kc1")
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{usekeywithclickevents.New()}, map[string]wrapperlint.Severity{"use-key-with-click-events": wrapperlint.SeverityError})
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-key-with-click-events" { count++ }
	}
	return count, nil
}
