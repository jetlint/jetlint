...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-header-scope.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nhs")
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{noheaderscope.New()}, map[string]wrapperlint.Severity{"no-header-scope": wrapperlint.SeverityError})
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-header-scope" { count++ }
	}
	return count, nil
}
