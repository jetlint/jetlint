...
func TestDotNotation_TypescriptEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/dot-notation.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "dot-notation")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
