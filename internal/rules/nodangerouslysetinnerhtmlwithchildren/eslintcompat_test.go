package nodangerouslysetinnerhtmlwithchildren_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nodangerouslysetinnerhtmlwithchildren"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-dangerously-set-inner-html-with-children.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-dhc-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nodangerouslysetinnerhtmlwithchildren.New()},
		map[string]wrapperlint.Severity{"no-dangerously-set-inner-html-with-children": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-dangerously-set-inner-html-with-children" {
			count++
		}
	}
	return count, nil
