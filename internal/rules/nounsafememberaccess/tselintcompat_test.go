package nounsafememberaccess_test
...
func TestNoUnsafeMemberAccess_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unsafe-member-access.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unsafe-member-access")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string, opts nounsafememberaccess.Options) (int, error) {
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unsafe-member-access" {
			count++
		}
	}
	return count, nil
