package noariaunsupportedelements_test
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-aria-unsupported-elements.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-aue")
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{noariaunsupportedelements.New()}, map[string]wrapperlint.Severity{"no-aria-unsupported-elements": wrapperlint.SeverityError})
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-aria-unsupported-elements" { count++ }
	}
	return count, nil
}
