...
func TestNoImpliedEval_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-implied-eval.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-implied-eval")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noimpliedeval.New()},
		map[string]wrapperlint.Severity{"no-implied-eval": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-implied-eval" {
			count++
		}
	}
	return count, nil
