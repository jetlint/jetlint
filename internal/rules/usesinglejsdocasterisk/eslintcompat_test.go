package usesinglejsdocasterisk_test
...
func TestUseSingleJsDocAsterisk_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-single-js-doc-asterisk.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-usjda-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{usesinglejsdocasterisk.New()},
		map[string]wrapperlint.Severity{"use-single-js-doc-asterisk": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-single-js-doc-asterisk" {
			count++
		}
	}
	return count, nil
