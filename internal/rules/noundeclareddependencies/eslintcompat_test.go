package noundeclareddependencies_test
...
func TestNoUndeclaredDependencies_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-undeclared-dependencies.json")
...
func runCase(t *testing.T, c eslintcompat.Case) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nud-")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	opts := &noundeclareddependencies.Options{DevDependencies: true}
	if first := c.FirstOption(); first != nil {
		if v, ok := first["devDependencies"].(bool); ok {
			opts.DevDependencies = v
		}
	}
	eng := engine.New(
		[]engine.Rule{noundeclareddependencies.New()},
		map[string]wrapperlint.Severity{"no-undeclared-dependencies": wrapperlint.SeverityError},
	).WithOptions(map[string]any{"no-undeclared-dependencies": opts})
	count := 0
	wantPath := filepath.Join(dir, mainPath)
	for _, d := range eng.Lint(prog) {
		if d.RuleID != "no-undeclared-dependencies" {
			continue
		}
