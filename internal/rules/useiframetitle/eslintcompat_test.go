...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-iframe-title.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-uft")
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{useiframetitle.New()}, map[string]wrapperlint.Severity{"use-iframe-title": wrapperlint.SeverityError})
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-iframe-title" { count++ }
	}
	return count, nil
}
