package noredundanttypeconstituents_test
...
func TestNoRedundantTypeConstituents_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-redundant-type-constituents.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-redundant-type-constituents")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noredundanttypeconstituents.New()},
		map[string]wrapperlint.Severity{"no-redundant-type-constituents": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-redundant-type-constituents" {
			count++
		}
	}
	return count, nil
