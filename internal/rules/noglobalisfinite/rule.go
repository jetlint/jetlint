// Package noglobalisfinite implements no-global-is-finite: global `isFinite`
// coerces its argument first (`isFinite("foo")` is `true`!). Use
// `Number.isFinite` for a check that doesn't lie.
package noglobalisfinite
...
func visit(ctx *engine.Context, n *wrapperchecker.Node) {
	...
	if isShadowed(n, "isFinite") {
		return
	}
	ctx.Report(n, "global `isFinite` coerces its argument — use `Number.isFinite`")
}
...
// a bare identifier, parenthesized, or `<global>.isFinite` / `<global>["isFinite"]`.
