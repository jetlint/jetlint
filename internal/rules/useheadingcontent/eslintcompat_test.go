package useheadingcontent_test
...
const ruleID = "use-heading-content"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New([]engine.Rule{useheadingcontent.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
