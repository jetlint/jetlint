package noconstantbinaryexpression_test
...
func TestNoConstantBinaryExpression_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-constant-binary-expression.json")
...
func runCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ncbe-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noconstantbinaryexpression.New()},
		map[string]wrapperlint.Severity{"no-constant-binary-expression": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-constant-binary-expression" {
			count++
		}
	}
	return count, nil
