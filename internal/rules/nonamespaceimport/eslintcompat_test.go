package nonamespaceimport_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nonamespaceimport"
)
...
func TestNoNamespaceImport_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-namespace-import.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nni-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nonamespaceimport.New()},
		map[string]wrapperlint.Severity{"no-namespace-import": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-namespace-import" {
			count++
		}
	}
	return count, nil
