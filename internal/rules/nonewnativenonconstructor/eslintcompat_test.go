...
func TestNoNewNativeNonconstructor_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-new-native-nonconstructor.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nnnc-compat")
	if err != nil {
		return 0, err
	}
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-new-native-nonconstructor" {
			count++
		}
	}
	return count, nil
