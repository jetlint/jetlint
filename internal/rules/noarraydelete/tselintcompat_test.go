...
func TestNoArrayDelete_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-array-delete.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-array-delete")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noarraydelete.New()},
		map[string]wrapperlint.Severity{"no-array-delete": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-array-delete" {
			count++
		}
	}
	return count, nil
