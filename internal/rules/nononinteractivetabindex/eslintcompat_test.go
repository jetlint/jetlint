package nononinteractivetabindex_test
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-noninteractive-tabindex.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nnt")
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{nononinteractivetabindex.New()}, map[string]wrapperlint.Severity{"no-noninteractive-tabindex": wrapperlint.SeverityError})
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-noninteractive-tabindex" { count++ }
	}
	return count, nil
}
