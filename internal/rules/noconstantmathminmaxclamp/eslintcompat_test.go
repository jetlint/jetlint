package noconstantmathminmaxclamp_test
...
func TestNoConstantMathMinMaxClamp_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-constant-math-min-max-clamp.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ncmmmc-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noconstantmathminmaxclamp.New()},
		map[string]wrapperlint.Severity{"no-constant-math-min-max-clamp": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-constant-math-min-max-clamp" {
			count++
		}
	}
	return count, nil
