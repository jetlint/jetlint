package requirearraysortcompare_test
...
func TestRequireArraySortCompare_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/require-array-sort-compare.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "require-array-sort-compare")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	var passed, failed int
	for _, c := range cases {
		opts := requirearraysortcompare.DefaultOptions()
		if v, ok := c.Options["ignoreStringArrays"].(bool); ok {
			opts.IgnoreStringArrays = v
		}
...
func runCase(t *testing.T, code string, opts requirearraysortcompare.Options) (int, error) {
	t.Helper()
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{requirearraysortcompare.NewWithOptions(opts)},
		map[string]wrapperlint.Severity{"require-array-sort-compare": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "require-array-sort-compare" {
			count++
		}
	}
	return count, nil
