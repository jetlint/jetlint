...
func TestAwaitThenable_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/await-thenable.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "await-thenable")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{awaitthenable.New()},
		map[string]wrapperlint.Severity{"await-thenable": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "await-thenable" {
			count++
		}
	}
	return count, nil
