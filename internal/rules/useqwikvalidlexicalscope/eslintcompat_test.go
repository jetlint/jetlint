package useqwikvalidlexicalscope_test
...
func TestUseQwikValidLexicalScope_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-qwik-valid-lexical-scope.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-uqvls-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{useqwikvalidlexicalscope.New()},
		map[string]wrapperlint.Severity{"use-qwik-valid-lexical-scope": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-qwik-valid-lexical-scope" {
			count++
		}
	}
	return count, nil
