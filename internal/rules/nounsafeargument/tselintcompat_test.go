...
func TestNoUnsafeArgument_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unsafe-argument.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unsafe-argument")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounsafeargument.New()},
		map[string]wrapperlint.Severity{"no-unsafe-argument": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unsafe-argument" {
			count++
		}
	}
	return count, nil
