...
func TestNoUnsafeReturn_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unsafe-return.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unsafe-return")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
