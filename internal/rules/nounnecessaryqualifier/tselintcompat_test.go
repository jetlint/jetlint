package nounnecessaryqualifier_test
...
func TestNoUnnecessaryQualifier_TypescriptEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unnecessary-qualifier.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unnecessary-qualifier")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounnecessaryqualifier.New()},
		map[string]wrapperlint.Severity{"no-unnecessary-qualifier": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unnecessary-qualifier" {
			count++
		}
	}
	return count, nil
