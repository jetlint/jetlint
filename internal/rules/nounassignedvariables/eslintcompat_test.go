package nounassignedvariables_test
...
func TestNoUnassignedVariables_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-unassigned-variables.json")
...
func runCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nounassigned")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounassignedvariables.New()},
		map[string]wrapperlint.Severity{"no-unassigned-variables": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unassigned-variables" {
			count++
		}
	}
	return count, nil
