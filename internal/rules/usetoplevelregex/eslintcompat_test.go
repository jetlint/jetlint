...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/usetoplevelregex"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/use-top-level-regex.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-utr-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{usetoplevelregex.New()},
		map[string]wrapperlint.Severity{"use-top-level-regex": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "use-top-level-regex" {
			count++
		}
	}
	return count, nil
