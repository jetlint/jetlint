...
func TestNoFallthrough_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-fallthrough.json")
...
func optionsForCase(c eslintcompat.Case) nofallthrough.Options {
	opts := nofallthrough.DefaultOptions()
	if len(c.Options) == 0 {
		return opts
	}
	...
	if v, ok := cfg["commentPattern"].(string); ok {
		opts.CommentPattern = v
	}
	if v, ok := cfg["allowEmptyCase"].(bool); ok {
		opts.AllowEmptyCase = v
	}
	if v, ok := cfg["reportUnusedFallthroughComment"].(bool); ok {
		opts.ReportUnusedFallthroughComment = v
	}
	return opts
}

func runEslintCase(t *testing.T, code string, opts nofallthrough.Options) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nf-compat")
	if err != nil {
		return 0, err
	}
