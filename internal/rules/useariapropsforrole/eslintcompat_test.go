package useariapropsforrole_test
...
const ruleID = "use-aria-props-for-role"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{useariapropsforrole.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
