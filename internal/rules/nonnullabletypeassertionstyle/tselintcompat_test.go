package nonnullabletypeassertionstyle_test
...
func TestNonNullableTypeAssertionStyle_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/non-nullable-type-assertion-style.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "non-nullable-type-assertion-style")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nonnullabletypeassertionstyle.New()},
		map[string]wrapperlint.Severity{"non-nullable-type-assertion-style": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "non-nullable-type-assertion-style" {
			count++
		}
	}
	return count, nil
