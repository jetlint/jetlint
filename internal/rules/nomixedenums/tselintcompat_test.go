...
func TestNoMixedEnums_TypescriptEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-mixed-enums.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-mixed-enums")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nomixedenums.New()},
		map[string]wrapperlint.Severity{"no-mixed-enums": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-mixed-enums" {
			count++
		}
	}
	return count, nil
