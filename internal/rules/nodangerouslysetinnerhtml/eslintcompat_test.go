package nodangerouslysetinnerhtml_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nodangerouslysetinnerhtml"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-dangerously-set-inner-html.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ndh-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nodangerouslysetinnerhtml.New()},
		map[string]wrapperlint.Severity{"no-dangerously-set-inner-html": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-dangerously-set-inner-html" {
			count++
		}
	}
	return count, nil
