package nonextasyncclientcomponent_test
...
func TestNoNextAsyncClientComponent_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-next-async-client-component.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nnacc-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nonextasyncclientcomponent.New()},
		map[string]wrapperlint.Severity{"no-next-async-client-component": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-next-async-client-component" {
			count++
		}
	}
	return count, nil
