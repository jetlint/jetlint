...
func TestNoUnsafeNegation_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-unsafe-negation.json")
...
func optionsForCase(c eslintcompat.Case) nounsafenegation.Options {
	var opts nounsafenegation.Options
	first := c.FirstOption()
	if first == nil {
		return opts
	}
	if v, ok := first["enforceForOrderingRelations"].(bool); ok {
		opts.EnforceForOrderingRelations = v
	}
	return opts
}

func runEslintCase(t *testing.T, code string, opts nounsafenegation.Options) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nun-compat")
	if err != nil {
		return 0, err
	}
