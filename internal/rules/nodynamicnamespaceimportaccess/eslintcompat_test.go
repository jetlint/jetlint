package nodynamicnamespaceimportaccess_test
...
import (
	...
	"github.com/jetlint/jetlint/internal/rules/nodynamicnamespaceimportaccess"
)
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-dynamic-namespace-import-access.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ndn-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nodynamicnamespaceimportaccess.New()},
		map[string]wrapperlint.Severity{"no-dynamic-namespace-import-access": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-dynamic-namespace-import-access" {
			count++
		}
	}
	return count, nil
