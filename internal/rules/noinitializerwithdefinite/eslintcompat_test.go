package noinitializerwithdefinite_test
...
func TestNoInitializerWithDefinite_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-initializer-with-definite.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-niwd-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noinitializerwithdefinite.New()},
		map[string]wrapperlint.Severity{"no-initializer-with-definite": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-initializer-with-definite" {
			count++
		}
	}
	return count, nil
