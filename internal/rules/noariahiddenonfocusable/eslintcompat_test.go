package noariahiddenonfocusable_test
...
func TestEslintCompatibility(t *testing.T) {
	...
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-aria-hidden-on-focusable.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ahf")
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{noariahiddenonfocusable.New()}, map[string]wrapperlint.Severity{"no-aria-hidden-on-focusable": wrapperlint.SeverityError})
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-aria-hidden-on-focusable" { count++ }
	}
	return count, nil
}
