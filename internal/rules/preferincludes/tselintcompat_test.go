...
func TestPreferIncludes_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/prefer-includes.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "prefer-includes")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{preferincludes.New()},
		map[string]wrapperlint.Severity{"prefer-includes": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "prefer-includes" {
			count++
		}
	}
	return count, nil
