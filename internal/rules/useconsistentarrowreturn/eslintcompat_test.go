package useconsistentarrowreturn_test
...
const ruleID = "use-consistent-arrow-return"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{useconsistentarrowreturn.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
