package novuedataobjectdeclaration_test
...
func TestNoVueDataObjectDeclaration_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-vue-data-object-declaration.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nvdod-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{novuedataobjectdeclaration.New()},
		map[string]wrapperlint.Severity{"no-vue-data-object-declaration": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-vue-data-object-declaration" {
			count++
		}
	}
	return count, nil
