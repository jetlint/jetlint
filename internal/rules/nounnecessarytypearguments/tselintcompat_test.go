...
func TestNoUnnecessaryTypeArguments_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unnecessary-type-arguments.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unnecessary-type-arguments")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unnecessary-type-arguments" {
			count++
		}
	}
	return count, nil
