package novuesetuppropsreactivityloss_test
...
func TestNoVueSetupPropsReactivityLoss_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-vue-setup-props-reactivity-loss.json")
...
func runEslintCase(t *testing.T, code string) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-nvsprl-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{novuesetuppropsreactivityloss.New()},
		map[string]wrapperlint.Severity{"no-vue-setup-props-reactivity-loss": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-vue-setup-props-reactivity-loss" {
			count++
		}
	}
	return count, nil
