package nointeractiveelementtononinteractiverole_test
...
const ruleID = "no-interactive-element-to-noninteractive-role"
...
func runEslintCase(t *testing.T, code string) (int, error) {
	...
	if err != nil { return 0, err }
	defer prog.Close()
	eng := engine.New([]engine.Rule{nointeractiveelementtononinteractiverole.New()}, map[string]wrapperlint.Severity{ruleID: wrapperlint.SeverityError})
	count := 0
