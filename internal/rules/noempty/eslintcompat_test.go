...
func TestNoEmpty_EslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/eslint/no-empty.json")
...
func optionsForCase(c eslintcompat.Case) noempty.Options {
	opts := noempty.DefaultOptions()
	first := c.FirstOption()
	if first == nil {
		return opts
	}
	if v, ok := first["allowEmptyCatch"].(bool); ok {
		opts.AllowEmptyCatch = v
	}
	return opts
}

func runEslintCase(t *testing.T, code string, opts noempty.Options) (int, error) {
	t.Helper()
	dir, err := os.MkdirTemp("/tmp", "jl-ne-compat")
	if err != nil {
		return 0, err
	}
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{noempty.NewWithOptions(opts)},
		map[string]wrapperlint.Severity{"no-empty": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-empty" {
			count++
		}
	}
	return count, nil
