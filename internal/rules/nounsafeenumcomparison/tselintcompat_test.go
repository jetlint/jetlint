package nounsafeenumcomparison_test
...
func TestNoUnsafeEnumComparison_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-unsafe-enum-comparison.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-unsafe-enum-comparison")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	defer prog.Close()
	eng := engine.New(
		[]engine.Rule{nounsafeenumcomparison.New()},
		map[string]wrapperlint.Severity{"no-unsafe-enum-comparison": wrapperlint.SeverityError},
	)
	count := 0
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-unsafe-enum-comparison" {
			count++
		}
	}
	return count, nil
