package nomeaninglessvoidoperator_test
...
func TestNoMeaninglessVoidOperator_TypescriptEslintCompatibility(t *testing.T) {
	if testing.Short() {
		t.Skip("compatibility harness skipped under -short")
	}
	fixturePath, _ := filepath.Abs("../../../testdata/typescript-eslint/no-meaningless-void-operator.test.ts")
	cases, err := tselintcompat.Load(fixturePath, "no-meaningless-void-operator")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func runCase(t *testing.T, code string) (int, error) {
	...
	for _, d := range eng.Lint(prog) {
		if d.RuleID == "no-meaningless-void-operator" {
			count++
		}
	}
	return count, nil
